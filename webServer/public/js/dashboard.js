...
function sortDashboardData() {
	dashboardDataBattery = [...window.dashboardDataRecent].sort(
		function (a, b) {
...
function cardElement(
	...
	if (batteryPercentage < 41) {
		batteryWarningIcon =
			'<img src="/assets/icons/batteryLow.svg" class="battery-alert" height=40 class="batteryWarningIcon">';
...
function renderCards(list) {
	...
	for (
		let i = 0;
		i < document.querySelectorAll("button.laptopCard.inactive").length;
		i++
	) {
		const usageTimeElement = document
			.querySelectorAll("button.laptopCard.inactive")
			[i].querySelector("span#usageTime");
	...
	for (
		let i = 0;
		i < document.querySelectorAll("button.laptopCard.active").length;
		i++
	) {
		const usageTimeElement = document
			.querySelectorAll("button.laptopCard.active")
			[i].querySelector("span#usageTime");
		updateUsageTime(usageTimeElement);
		sessionTimerInterv = setInterval(
			updateUsageTime,
			1000,
			usageTimeElement,
		);
	...
	if (document.querySelector(".cardRow")) {
		document.getElementById("noRecords").classList.add("hide");
	};
}
...
function updateUsageTime(tElement) {
	const loginTime = new Date(
		tElement.getAttribute("data-datetime").replace(" ", "T"),
	);
	let now = new Date();
...
function formatLastUsed(tElement) {
	...
	if (logoutTime.toDateString() === now.toDateString()) {
		...
		const month = logoutTime.toLocaleDateString("en-US", {
			month: "short",
		});
...
function showInfoModal(pos) {
	...
	if (!modalElement.open) {
		...
		document.getElementById("modalUser").innerText =
			`${dbData.user.firstName} ${dbData.user.lastName}`;

		document.getElementById("modalBatteryIcon").src =
			`/assets/icons/battery${batteryIcon(dbData.battery)}.svg`;
		document.getElementById("modalBattery").innerText =
			`${dbData.battery}%`;
		document.getElementById("modalWifi").innerText = `${dbData.wifi}`;

		if (dbData.active) {
			document.getElementById("modalTimeName").innerText = "Usage Time:";
			document.getElementById("modalTimeIcon").src =
				`/assets/icons/usageTime.svg`;
			document
				.getElementById("modalTime")
				.setAttribute("data-datetime", dbData.user.loginTime);
			updateUsageTime(document.getElementById("modalTime"));
		} else {
			document.getElementById("modalTimeName").innerText = "Logout Time:";
			document.getElementById("modalTimeIcon").src =
				`/assets/icons/lastUsed.svg`;
			document
				.getElementById("modalTime")
				.setAttribute("data-datetime", dbData.lastPing);
...
function closeModal() {
	...
	const tableSortScript = document.querySelector(
